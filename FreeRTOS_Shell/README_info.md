Here's a useful set of diagnostic shell commands covering the most important FreeRTOS runtime info:

---

## 1. Stack watermarks (you have this already)
```cpp
void printStackWatermarks(void)
{
...
## 2. Task states — see what every task is doing right now
```cpp
...
## 3. Heap stats — free/used/fragmentation
```cpp
...
```
`xMinimumEverFreeBytesRemaining` is the most useful — it shows the **worst-case heap usage** since boot.
...
## 6. Combine them all into one `sysinfo` command
```cpp
...
```c
#define configUSE_TRACE_FACILITY                1   // for task states + uxTaskGetSystemState
#define configUSE_STATS_FORMATTING_FUNCTIONS    0   // don't need, we format ourselves
#define INCLUDE_uxTaskGetStackHighWaterMark     1   // already have this
#define INCLUDE_xTaskGetSchedulerState          1   // already have this
```

`vPortGetHeapStats()` requires `heap_4.c` or `heap_5.c` — which you're already using.