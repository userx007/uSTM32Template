...
use heapless::String;
use ushell_logger::{log_error, log_info};
...
mod async_impl {
        ...
        Y: core::future::Future<Output = ()>,
    {
        ...
        /// );
        /// ```
        pub fn new(try_read_fn: F, yield_fn: fn() -> Y, yield_threshold: u32) -> Self {
            Self {
                try_read_fn,
                yield_fn,
                empty_count: 0,
...
pub struct ShellConfig<const IML: usize, const EBS: usize> {
    ...
    pub command_dispatcher:
        for<'a> fn(&'a str, &'a mut String<EBS>) -> Result<(), &'a str>,
    pub shortcut_dispatcher:
        for<'a> fn(&'a str, &'a mut String<EBS>) -> Result<(), &'a str>,
...
fn exec<const IML: usize, const EBS: usize>(
    input: &String<IML>,
    is_shortcut: fn(&str) -> bool,
    command_dispatcher: for<'a> fn(
        &'a str,
        &'a mut String<EBS>,
    ) -> Result<(), &'a str>,
    shortcut_dispatcher: for<'a> fn(
        &'a str,
        &'a mut String<EBS>,
    ) -> Result<(), &'a str>,
) {
    ...
    let result: Result<(), &str> = if is_shortcut(input_str) {
